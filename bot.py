...
def main():
    ...
    async def enviar_mensagem(event):
        if event.media:
            await client.send_file(-1002012170582,file = event.media,
                                   caption=event.raw_text.replace('http://www.seinscrevanocanal.com.br','http://www.deixeseulike.com.br'))
        else:
            await client.send_message(-1002012170582,event.raw_text.replace('http://www.seinscrevanocanal.com.br','http://www.deixeseulike.com.br'))
    client.start()
